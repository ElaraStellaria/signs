...
var datos: Dictionary = {
	...
	"inventario": {
			"señales":     [],   # [1, 2, 3] — IDs de señales encontradas
			"consumibles": [],   # [{"id": "medialunas", "cantidad": 2}]
			"equipamiento": {    # slots fijos según el GDD
			"slot_1": null,  # accesorio 1
			"slot_2": null,  # accesorio 2  
			"slot_3": null,  # accesorio 3
			},
			"fragmentos":  [],   # [10, 11] — IDs de fragmentos de memoria
			},
	"jugador": {
...
func get_inventario(clave: String):
	return datos["inventario"].get(clave)

func set_inventario(clave: String, valor) -> void:
	datos["inventario"][clave] = valor
